...
export class TaskController {
  ...
  @Get(':listTaskId')
  async getAllTasks(
    @Param('listTaskId') listTaskId: number,
  ): Promise<ResponsesSuccessInterface | ResponsesErrorInterface> {
    try {
      const getAllTasks = await this.taskService.getTasksForListTask(
        listTaskId,
      );
  ...
  async updateTaskStatus(
    ...
    console.log(
      'task.controller .ts ~ updateStatusTask id - status',
      id,
      status,
    );

    try {
      const updateStatusTask = await this.taskService.updateStatusTask(
        id,
        status,
      );
      console.log(
        'task.controller .ts ~ updateStatusTask updateStatusTask',
        updateStatusTask,
      );
      ...
      return {
        status: 200,
        error: false,
        message: 'Status tasks has been updated',
        results: updateStatusTask,
      };
    } catch (error) {
      return { status: 500, error: true, message: error };
    }
