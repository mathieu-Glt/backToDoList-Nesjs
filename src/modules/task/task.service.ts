...
import { ListTaskService } from 'src/modules/listTasks/listTask.service';
  ...
  async getTasksForListTask(listTaskId: number): Promise<Task[] | {}> {
    try {
      ...
      throw new InternalServerErrorException(
        'Failed to get all tasks associate associated with their task list',
      );
    }
  }
  ...
  async deleteTask(id: number): Promise<object | InternalServerErrorException> {
    try {
      ...
      if (result.affected === 0)
        throw new NotFoundException(`List task with ID ${id} not found`);
